...
public class RespawnManager {
    ...
    public RespawnManager(EnderHunt plugin, GameManager gameManager, KitManager kitManager) {
        this.plugin = plugin;
    ...
    public void startRespawn(Player player) {
        ...
        BukkitRunnable respawnTask = new BukkitRunnable() {
            int timeLeft = plugin.getConfig().getInt("hunter-respawn-time");;
        ...
        // 发送初始消息
        player.sendTitle("§c☠ 你已死亡", "§7等待重生...", 10, 60, 20);
        player.sendMessage("§c你已死亡！将在 §e" + plugin.getConfig().getInt("hunter-respawn-time") + "秒 §c后重生");
    }
    ...
    private void sendRespawnMessage(Player player, int timeLeft) {
        if (timeLeft == plugin.getConfig().getInt("hunter-respawn-time")) {
